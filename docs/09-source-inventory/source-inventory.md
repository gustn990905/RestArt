...
source inventory 정리 과정에서는 기존 자료를 단순히 파일 단위로 나열하는 것이 아니라, 각 자료가 어떤 기능과 연결되는지 정리한다.
...
| Source Inventory            | source inventory 정리 중                   |
...
후속 구현 단계에서는 이 인벤토리를 기준으로 다음 작업을 진행한다.
